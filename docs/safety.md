...
Managed roles and the `@everyone` server role cannot be updated. Resource creates, channel moves, move-and-updates, and reorders are unsupported in v1.0 and abort the entire apply. Supported writes are existing role updates, overwrite creation/update, and channel permission synchronization. Ambiguous channel migration matches are blocking and never produce replacement creates.
